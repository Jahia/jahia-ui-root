...
const createTestrailRun = async (
    ...
    let run = await api.addRun(projectId, {
        name: runName,
        milestone_id: milestoneId, // eslint-disable-line
        description: runDescription,
        include_all: false, // eslint-disable-line
        case_ids: caseIds // eslint-disable-line
    });
    run = run.body;
    return run;
};
...
const createTestcase = async (api, testcaseTitle, section, notes) => {
    ...
    let testcase = await api.addCase(section.id, {
        title: testcaseTitle,
        custom_status: 2, // eslint-disable-line
        custom_notes: notes // eslint-disable-line
    });
        ...
        if (apiObj !== null) {
            ...
            for (const suiteResult of results.testResults) {
                ...
                if (suiteResult.testResults.length > 0) {
                    ...
                    const section = await createSection( // eslint-disable-line
                        apiObj,
                        project,
                        suiteResult.testResults[0].ancestorTitles[0]
                    );
                    const existingTestCases = await getTestCases( // eslint-disable-line
                        apiObj,
                        project
                    );
                    for (const testResult of suiteResult.testResults) {
                        ...
                        if (currentCase === undefined) {
                            currentCase = await createTestcase( // eslint-disable-line
                                apiObj,
                                testResult.title,
                                section,
                                'graphQL query'
                        ...
                        sendResults.push({
                            case_id: currentCase.id, // eslint-disable-line
                            status_id: testResult.status === 'passed' ? 1 : 5, // eslint-disable-line
                            comment:
